...
public final class ViewPropsJSIDecoder: NSObject {
  ...
  /**
   ...
   definitions for `className`, reads each JS-thread-decodable prop's value off the props
   object, decodes it through the regular dynamic-type machinery, and returns the decoded
   values boxed in a `DecodedViewProps`. Props whose types are not safe to decode on the JS
   thread are skipped here and left for the legacy main-thread dictionary path.
